...
in
{
  ...
  config = {
      ...
      mapAttrs'
        (name: backup:
          let
            ...
            filesFromTmpFile = "/tmp/restic-backups-${name}/includes";
            ...
            preStart = ''
              ...
              ${optionalString (backup.paths != null && backup.paths != []) ''
                mkdir -p /tmp/restic-backups-${name}
          ...
          nameValuePair "restic-backups-${name}" ({
            environment = {
              ...
              RESTIC_CACHE_DIR = "/tmp/cache/restic-backups-${name}";
            ...
            script = preStart +
                lib.concatStringsSep "\n" ((optionals doBackup ["${resticCmd} backup ${concatStringsSep " " (backup.extraBackupArgs ++ excludeFlags)} --files-from=${filesFromTmpFile}"])
                ++ pruneCmd)
                + postStop;
            serviceConfig = {
              LowPriorityIO = true;
    ...
    in pkgs.writeShellScriptBin "restic-${name}" ''
      ...
      ${lib.pipe config.launchd.user.agents."restic-backups-${name}".environment [
       (lib.filterAttrs (n: v: v != null && n != "PATH"))
       (lib.mapAttrsToList (n: v: "${n}=${v}"))
       (lib.concatStringsSep "\n")
      ]}
      PATH=${config.launchd.user.agents."restic-backups-${name}".path}:$PATH
